// src/db/Database.ts
// SQLite — wallets, sessions, assets, identities, conversation states,
// fingerprints, pending shares, access tokens.
...
const SCHEMA = `
  ...
  -- When owner shares with a username that hasn't registered yet.
  ...
  -- One-time or multi-use signed download links.
  -- No NEXAR account needed to use.
...
export type AssetRow             = { id: number; asset_id: string | null; ip_id: string; vault_uuid: string; vault_cid: string | null; tier: number; owner: string; name: string; asset_type: string; active: number; created_at: number; };
...
export type FingerprintRow       = { ip_id: string; owner: string; asset_name: string; sha256: string; phash: string | null; fingerprints: string | null; file_type: string; mime_type: string | null; created_at: number; };
